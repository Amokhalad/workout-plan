  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-4xl gap-2 p-2">
        {(['workouts', 'overview'] as const).map((v) => (
          <button
            key={v}
            type="button"
            onClick={() => onChange(v)}
            className={`touch-target flex flex-1 items-center justify-center rounded-xl py-3 text-sm font-semibold capitalize transition active:scale-[0.98] ${
              view === v
